...
**图 7.6 对输入 $\mathbf { x } _ { i }$ 进行多头注意力计算，产生输出 $\mathbf { a } _ { i }$。一个多头注意力层包含 A 个注意力头，每个头都有自己的查询、键和值权重矩阵。为了能在页面中容纳，本图令 $A = 4$，这个数值小于通常采用的头数。每个注意力头的输出形状为 $[ 1 \times d _ { \nu } ]$；这些输出被拼接起来，再由矩阵 ${ \boldsymbol { \mathsf { W } } } _ { 0 }$ 投影到另一个空间。通常会设置各头的维度 $d _ { \nu }$，使 $d _ { \nu } = d / A$，于是 $\boldsymbol { \mathsf { W } } _ { 0 }$ 是形状为 $[ A d _ { \nu } \times d ] = [ d \times d ]$ 的方阵。最终结果被投影到 d 维，因而输出与输入大小相同。**
...
我们已经介绍了注意力层，下面将在处理词元位置 i 上单个输入 $\pmb { x } _ { i }$ 的语境中，介绍前馈计算和层归一化计算。
...
通常会将前馈网络隐藏层的维度 $d _ { \mathrm { f f } }$ 设为模型维度 $d$ 的 4 倍。（例如，在最初的 Transformer 模型中，$d = 5 1 2$，而 $d _ { \mathrm { f f } } = 2 0 4 8$。）这意味着前馈网络占据了 Transformer 中的大多数参数，而且这些前馈网络参数似乎编码了 Transformer 中大部分事实知识（Geva et al., 2021; Meng et al., 2022）。
...
层归一化可以看作统计学中 z 分数的一种变体，它作用于隐藏层里的单个向量。也就是说，“层归一化”这个名称有些容易引起误解：它并不是应用于整个 Transformer 层，而只是应用于单个词元的嵌入向量。因此，层归一化的输入是一个维度为 d 的向量，输出则是归一化后的同一个向量，维度仍为 $d$。层归一化的第一步，是在待归一化向量的各个元素上计算均值 $\mu$ 和标准差 <sub>σ</sub>。给定一个维度为 $d$ 的嵌入向量 x，这两个量按如下方式计算。
...
至关重要的是，Transformer 块的输入和输出维度保持一致，因此可以堆叠。输入到块中的每个词元向量 $\pmb { x } _ { i }$ 的维度为 d，输出 $\mathbf { h } _ { i }$ 的维度同样为 d。用于大语言模型的 Transformer 会堆叠许多这样的块：从 12 层（用于 T5 或 GPT-3-small 语言模型），到 96 层（用于 GPT-3 175B），较新的模型甚至会使用更多层。稍后我们还会回到这个堆叠问题。
...
堆叠许多个块以后，还需要满足一项要求：在最后一个（最高的）Transformer 块末端，需要对每条词元流最后的 h<sub>i</sub> 额外执行一次层归一化（位置就在我们稍后会定义的语言模型头层下方）。[^2]
