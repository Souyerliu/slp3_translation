...
$$
\begin{array}{rcl}\frac{\partial\operatorname{Cost}(\hat y,y)}{\partial\mathbf w}&=&\frac1m(\hat{\mathbf y}-\mathbf y)^\intercal\mathbf X\\&=&\frac1m(\sigma(\mathbf X\mathbf w+\mathbf b)-\mathbf y)^\intercal\mathbf X\end{array}\tag{4.32}
$$
